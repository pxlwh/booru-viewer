...
Download `booru-viewer-setup.exe` from [Releases](../../releases)(hosted on gitea) and run the installer. It installs to AppData with Start Menu and optional desktop shortcuts. To update, just run the new installer over the old one — your data in `%APPDATA%\booru-viewer\` is preserved.
